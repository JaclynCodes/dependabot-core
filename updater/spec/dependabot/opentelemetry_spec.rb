...
RSpec.describe Dependabot::OpenTelemetry do
  ...
  describe "edge cases and regression tests" do
    ...
    describe "OpenTelemetry configuration edge cases" do
      it "handles missing OTEL environment variables gracefully" do
        # Save original values
        original_service_name = ENV["OTEL_SERVICE_NAME"]
        original_exporter = ENV["OTEL_EXPORTER_OTLP_ENDPOINT"]
        ...
        begin
          ...
          # Should still be able to check configuration status
          expect { described_class.should_configure? }.not_to raise_error
        ensure
          # Restore original values
          ENV["OTEL_SERVICE_NAME"] = original_service_name if original_service_name
          ENV["OTEL_EXPORTER_OTLP_ENDPOINT"] = original_exporter if original_exporter
        end
      end

      it "handles rapid configure/shutdown cycles without errors" do
        tracer_provider = instance_double(::OpenTelemetry::SDK::Trace::TracerProvider)
        allow(::OpenTelemetry::SDK).to receive(:configure).and_yield(double(
          service_name: nil,
          use: nil
        ...
        # Perform multiple configure/shutdown cycles
        3.times do
